...
equities = ['BAJFINANCE', 'INFY', 'RELIANCE', 'ICICIBANK', 'BAJAJFINSERV', 'ITC', 'TATAMOTORS', 'HCLTECH', 'TCS',
            'MARUTI']
            ...
            for n in range(n_days):
                if df.loc[i, 'close_price'] - df.loc[i + n, 'close_price'] >= fraction_movement * df.loc[
                    ...
                    if i >= 20:
                        ...
                        fig.savefig('/Users/animeshupreti/PycharmProjects/Candlestick Identifier/Candle Data/Down/{'
                                    '0}{1}.png'.format(df_pricing['Symbol'][i], i), dpi=70)
                    print('Down', i, n)
                    break
                    ...
                    if i > 20:
                        fig = plot_candles(df_pricing[i - 20:i], volume_bars=True)
                        fig.savefig('/Users/animeshupreti/PycharmProjects/Candlestick Identifier/Candle Data/Up/{0}{'
                                    '1}.png'.format(df_pricing['Symbol'][i], i), dpi=70)
                    print('Up', i, n)
                    break
                else:
...
path = '/Users/animeshupreti/PycharmProjects/Candlestick Identifier/Candle Data'
path_save = '/Users/animeshupreti/PycharmProjects/Candlestick Identifier/Processed'

path.ls()

np.random.seed(42)
data = ImageDataBunch.from_folder(path, train=".", valid_pct=0.2,
                                  ds_tfms=get_transforms(flip_vert=False, max_lighting=0.1, max_zoom=1.05, max_warp=0.,
                                                         max_rotate=3), size=224, num_workers=4).normalize(
    imagenet_stats)
...
data = ImageDataBunch.from_folder(path, train=".", valid_pct=0.2,
                                  ds_tfms=get_transforms(flip_vert=False, max_lighting=0.1, max_zoom=1.05, max_warp=0.,
                                                         max_rotate=3), size=352, num_workers=4).normalize(
    imagenet_stats)
